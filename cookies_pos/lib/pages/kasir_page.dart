...
class _KasirPageState extends State<KasirPage> with AutomaticKeepAliveClientMixin {
      ...
      child: Column(
        ...
        children: [
          ...
          if (cartEntries.isEmpty)
            ...
            ...cartEntries.map((entry) => _buildCartItem(entry.key, entry.value)),
  ...
  Widget _buildCartItem(int productId, int qty) {
    if (!_products.any((p) => p.id == productId)) return const SizedBox.shrink();
    final product = _products.firstWhere((p) => p.id == productId);
    return Padding(
              ...
              children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    height: 28,
                    decoration: const BoxDecoration(shape: BoxShape.circle),
                    child: Icon(Icons.add, size: 16, color: product.stock > qty ? AppTheme.primary : AppTheme.onSurfaceVariant.withValues(alpha: 0.5)),
                  ),
                ),
              ],
            ),
